...
class ObjectiveEvaluationDispatcher(ABC):
    ...
    Defines technical detail of how objective evaluation for population must be processed.
    Semantically it's just a mapper for objective function that handles:
    - implementation-specific evaluation policy (e.g. sequential,parallel, async);
...
class MultiprocessingDispatcher(ObjectiveEvaluationDispatcher):
    ...
    For using it call `dispatch(objective_function)` to get evaluation function."""
    ...
    def __init__(self,
                 ...
                 log: Log = None,
                 n_jobs: int = 1,
                 cleanup_fn: Optional[GraphFunction] = None,
                 post_eval_callback: Optional[GraphFunction] = None):
        self._objective_eval = None
        self._graph_adapter = graph_adapter
        self._post_eval_callback = post_eval_callback
